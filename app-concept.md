...
/agent
  ...
  /src                   # next version of itself; the only writable area
    registry.<ext>       # central name→adapter registry (tools, providers, backends); off-limits under improve:tools
    /providers           # one file per LLM provider (openrouter, ollama, ...)
    /tools               # agent tool backends (the model calls these)
      /search            #   one file per web_search backend (tavily, exa, ...)
      /fetch             #   one file per web_fetch  backend (jina, firecrawl, ...)
...
Standard ReAct/tool-calling loop. Parsing and JSON-schema validation are delegated to the LLM SDK's native tool-calling API — no custom parser needed.
...
`llm.call(...)` in the loop is a thin dispatch over a **provider adapter**. One file per provider lives in `/src/providers/<name>.<ext>`; the registry maps `llm.provider: <name>` to its adapter. v1 ships with **openrouter** only — since OpenRouter itself proxies most major models, this already covers most users.
...
Same pattern as LLM providers, applied to the two web tools. `tools.web_search` and `tools.web_fetch` each name a backend; one file per backend lives in `/src/tools/search/<name>.<ext>` and `/src/tools/fetch/<name>.<ext>`. v1 ships with **tavily** (search) and **jina** (fetch) only.
...
- `run_tests` — run the agent's `/src/tests` suite against `/agent/src` (sandboxed subprocess); the deploy gate runs it automatically too
- `install` — install a package from the configured registry into the agent's environment (a bounded capability, not a shell). Default-allow for ordinary packages; package-manager / system / off-registry installs need human approval (see Security). The dependency set is recorded and validated through the deploy gate.
...
The agent may add new tools by adding the implementation under `/src` and an entry in the central `registry` file (`/src/registry.<ext>`), then redeploying. Until they pass the next deploy and the runtime's sandbox check, they aren't callable.
...
- **Configured hosts** — the hosts of currently configured providers/backends plus the package registry (e.g. `openrouter.ai`, `api.tavily.com`, `r.jina.ai`, and the registry). Full request methods, so a proxy backend may `POST` its query here. Adding a host to this set is a capability-frontier change (human-extended).
...
**Dependencies.** The agent may install packages to build itself, exposed as a bounded `install` capability (not a shell). It is language-independent for now — the concrete package manager and registry are chosen with the language, later. Installs are **default-allow** from the single configured registry (its host is the one egress addition for installs); each is logged and rides a **deploy**, so the dependency set is test-gated and reproducible; a rollback reverts the dependency *manifest* (already-installed packages may linger in the environment, harmlessly — the reverted code simply won't import them). The **restricted set** needs human approval (it crosses the capability frontier): installing or replacing the package manager itself, system-level / app installs, and any registry other than the configured one. Packages widen what the agent can *build*, not what its code may *do* — the sandbox still governs that. In production, dependency changes follow `improve` (full only).

**Sandboxed tests.** The smoke floor and the agent's `/src/tests` run in a subprocess with the same filesystem + network restrictions.
...
- `run_tests` — sandboxed subprocess
- `install` — install a dependency from the configured registry (bounded; package-manager / system / off-registry installs need approval — see Security)
